import { z } from "zod";
import { SECRET_BINDING_TARGET_TYPES, SECRET_MANAGED_MODES, SECRET_PROVIDER_CONFIG_STATUSES, SECRET_PROVIDERS, SECRET_STATUSES, } from "../constants.js";
...
const deniedProviderConfigKeyPattern = /^(access[-_]?key([-_]?id)?|secret[-_]?access[-_]?key|secret[-_]?key|token|password|passwd|credential|credentials|private[-_]?key|pem|jwt|session[-_]?token|service[-_]?account([-_]?json)?|client[-_]?secret|secret[-_]?id|unseal[-_]?key|recovery[-_]?key|key[-_]?file([-_]?path)?|token[-_]?file([-_]?path)?)$/i;
...
const vaultAddressSchema = z.preprocess((value) => typeof value === "string" ? value.trim() : value, z.string().url().superRefine((value, ctx) => {
    let url;
    try {
        url = new URL(value);
    }
