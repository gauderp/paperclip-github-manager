...
export function buildIssueReferenceHref(identifier) {
    const normalized = normalizeIssueIdentifier(identifier);
    return `/issues/${normalized ?? identifier.trim()}`;
}
export function parseIssueReferenceHref(href) {
    const raw = href.trim();
    if (!raw)
        return null;
    let url;
    ...
    for (let index = 0; index < segments.length - 1; index += 1) {
        if (segments[index]?.toLowerCase() !== "issues")
            continue;
...
export function findIssueReferenceMatches(text) {
    if (!text)
        return [];
    const matches = [];
    let match;
    ...
    while ((match = re.exec(text)) !== null) {
        ...
        if (!cleanedToken)
            continue;
        const identifier = normalizeIssueIdentifier(cleanedToken)
...
export function extractIssueReferenceIdentifiers(markdown) {
...
export function extractIssueReferenceMatches(markdown) {
    const scrubbed = stripMarkdownCode(markdown);
    const seen = new Set();
    const ordered = [];
    for (const match of findIssueReferenceMatches(scrubbed)) {
...
//# sourceMappingURL=issue-references.js.map